...
- Em alguns cenários, prefira a URL pooler (`aws-0-us-east-1.pooler.supabase.com:6543`) para melhorar estabilidade de conexão em ambiente serverless.
...
## 12) Dump e configuração de banco para entrega ao cliente

Este projeto já inclui:
- `db/schema.sql` (estrutura SQL do banco)
- `db/README_DUMP.md` (comandos de geração e aplicação)

### 12.1 Dump de estrutura (já pronto)
Arquivo:
- `db/schema.sql`
...
### 12.2 Dump completo (estrutura + dados) via PostgreSQL
Se quiser exportar também os dados do ambiente atual:
```bash
pg_dump "$DATABASE_URL" --no-owner --no-privileges > db/full_dump.sql
```
...
### 12.3 Arquivo de conexão para o cliente
Entregar ao cliente um `.env.example` com este formato:
```env
DATABASE_URL="postgresql://postgres:<SENHA_URL_ENCODED>@db.<PROJECT_REF>.supabase.co:5432/postgres?sslmode=require"
```
